...
// @ts-expect-error
sjcl_1.default.beware["CBC mode is dangerous because it doesn't protect message integrity."]();
const Aes = sjcl_1.default.cipher.aes;
class AesCmac {
    static Bsize = 128; // in bits! not octets (16)
    static Zero = sjcl_1.default.codec.hex.toBits('0x00000000000000000000000000000000');
    static Rb = sjcl_1.default.codec.hex.toBits('0x00000000000000000000000000000087');
    cipher;
    /**
     * @param key Key as Hex string
     */
    constructor(key) {
        const keyBits = sjcl_1.default.codec.hex.toBits(key);
    ...
    simpleShiftLeft = (a, shift) => {
        return sjcl_1.default.bitArray.bitSlice(sjcl_1.default.bitArray.concat(a, [0]), shift, AesCmac.Bsize + shift);
    };
    /**
     * @param message Message as `BitArray`
     */
    iso7816d4Padding = (message) => {
        const bitLength = sjcl_1.default.bitArray.bitLength(message);
     ...
     */
    encrypt = (message) => {
        return sjcl_1.default.bitArray.clamp(sjcl_1.default.mode.cbc.encrypt(this.cipher, message, AesCmac.Zero), AesCmac.Bsize);
    };
    /**
    ...
    generateCmac = (plainText) => {
        ...
        // Step 2
        const M = sjcl_1.default.codec.hex.toBits(plainText);
        const len = sjcl_1.default.bitArray.bitLength(M); // in bits! not octets
        ...
        let last = sjcl_1.default.bitArray.bitSlice(M, lastStart);
        ...
        for (let i = 1; i <= n - 1; i++) {
            ...
            const mI = sjcl_1.default.bitArray.bitSlice(M, start, end);
