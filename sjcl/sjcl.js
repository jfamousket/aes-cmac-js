...
sjcl.cipher.aes = function (a) {
  this.s[0][0][0] || this.O(); let b; let c; let d; let e; const f = this.s[0][4]; const g = this.s[1]; b = a.length; let h = 1; if (b !== 4 && b !== 6 && b !== 8) throw new sjcl.exception.invalid('invalid aes key size'); this.b = [d = a.slice(0), e = []]; for (a = b; a < 4 * b + 28; a++) { c = d[a - 1]; if (a % b === 0 || b === 8 && a % b === 4)c = f[c >>> 24] << 24 ^ f[c >> 16 & 255] << 16 ^ f[c >> 8 & 255] << 8 ^ f[c & 255], a % b === 0 && (c = c << 8 ^ c >>> 24 ^ h << 24, h = h << 1 ^ 283 * (h >> 7)); d[a] = d[a - b] ^ c } for (b = 0; a; b++, a--) {
    c = d[b & 3 ? a : a - 4], e[b] = a <= 4 || b < 4
      ? c
      : g[0][f[c >>> 24]] ^ g[1][f[c >> 16 & 255]] ^ g[2][f[c >> 8 & 255]] ^ g[3][f[c &
255]]
  }
}
...
sjcl.bitArray = {
  ...
  extract: function (a, b, c) { const d = Math.floor(-b - c & 31); return ((b + c - 1 ^ b) & -32 ? a[b / 32 | 0] << 32 - d ^ a[b / 32 + 1 | 0] >>> d : a[b / 32 | 0] >>> d) & (1 << c) - 1 },
  concat: function (a, b) { if (a.length === 0 || b.length === 0) return a.concat(b); const c = a[a.length - 1]; const d = sjcl.bitArray.getPartial(c); return d === 32 ? a.concat(b) : sjcl.bitArray.$(b, d, c | 0, a.slice(0, a.length - 1)) },
  bitLength: function (a) {
    const b = a.length; return b ===
0
      ? 0
      : 32 * (b - 1) + sjcl.bitArray.getPartial(a[b - 1])
  },
  clamp: function (a, b) { if (32 * a.length < b) return a; a = a.slice(0, Math.ceil(b / 32)); const c = a.length; b = b & 31; c > 0 && b && (a[c - 1] = sjcl.bitArray.partial(b, a[c - 1] & 2147483648 >> b - 1, 1)); return a },
...
0
  ...
  $: function (a, b, c, d) { let e; e = 0; for (void 0 === d && (d = []); b >= 32; b -= 32)d.push(c), c = 0; if (b === 0) return d.concat(a); for (e = 0; e < a.length; e++)d.push(c | a[e] >>> b), c = a[e] << 32 - b; e = a.length ? a[a.length - 1] : 0; a = sjcl.bitArray.getPartial(e); d.push(sjcl.bitArray.partial(b + a & 31, b + a > 32 ? c : d.pop(), 1)); return d },
  f: function (a, b) { return [a[0] ^ b[0], a[1] ^ b[1], a[2] ^ b[2], a[3] ^ b[3]] },
  byteswapM: function (a) { let b, c; for (b = 0; b < a.length; ++b)c = a[b], a[b] = c >>> 24 | c >>> 8 & 0xff00 | (c & 0xff00) << 8 | c << 24; return a }
}
sjcl.codec.utf8String = { fromBits: function (a) { let b = ''; const c = sjcl.bitArray.bitLength(a); let d; let e; for (d = 0; d < c / 8; d++)(d & 3) === 0 && (e = a[d / 4]), b += String.fromCharCode(e >>> 8 >>> 8 >>> 8), e <<= 8; return decodeURIComponent(escape(b)) }, toBits: function (a) { a = unescape(encodeURIComponent(a)); const b = []; let c; let d = 0; for (c = 0; c < a.length; c++)d = d << 8 | a.charCodeAt(c), (c & 3) === 3 && (b.push(d), d = 0); c & 3 && b.push(sjcl.bitArray.partial(8 * (c & 3), d)); return b } }
sjcl.codec.hex = { fromBits: function (a) { let b = ''; let c; for (c = 0; c < a.length; c++)b += ((a[c] | 0) + 0xf00000000000).toString(16).substr(4); return b.substr(0, sjcl.bitArray.bitLength(a) / 4) }, toBits: function (a) { let b; const c = []; let d; a = a.replace(/\s|0x/g, ''); d = a.length; a = a + '00000000'; for (b = 0; b < a.length; b += 8)c.push(parseInt(a.substr(b, 8), 16) ^ 0); return sjcl.bitArray.clamp(c, 4 * d) } }
sjcl.codec.base32 = {
  ...
  BASE: 5,
  REMAINING: 27,
  fromBits: function (a, b, c) { const d = sjcl.codec.base32.BASE; const e = sjcl.codec.base32.REMAINING; let f = ''; let g = 0; let h = sjcl.codec.base32.B; let k = 0; const l = sjcl.bitArray.bitLength(a); c && (h = sjcl.codec.base32.X); for (c = 0; f.length * d < l;)f += h.charAt((k ^ a[c] >>> g) >>> e), g < d ? (k = a[c] << d - g, g += e, c++) : (k <<= d, g -= d); for (;f.length & 7 && !b;)f += '='; return f },
  toBits: function (a, b) {
...
sjcl.codec.base64 = {
  ...
  fromBits: function (a, b, c) { let d = ''; let e = 0; let f = sjcl.codec.base64.B; let g = 0; const h = sjcl.bitArray.bitLength(a); c && (f = f.substr(0, 62) + '-_'); for (c = 0; 6 * d.length < h;)d += f.charAt((g ^ a[c] >>> e) >>> 26), e < 6 ? (g = a[c] << 6 - e, e += 26, c++) : (g <<= 6, e -= 6); for (;d.length & 3 && !b;)d += '='; return d },
  toBits: function (a, b) {
...
sjcl.hash.sha256.prototype = {
  ...
  finalize: function () { let a; var b = this.A; const c = this.F; var b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]); for (a = b.length + 2; a & 15; a++)b.push(0); b.push(Math.floor(this.l / 0x100000000)); for (b.push(this.l | 0); b.length;)u(this, b.splice(0, 16)); this.reset(); return c },
  Y: [],
  b: [],
  O: function () {
...
sjcl.mode.ccm = {
  ...
  fa: function (a) { const b = sjcl.mode.ccm.G.slice(); let c; for (c = 0; c < b.length; c += 1)b[c](a) },
  ...
  C: function (a, b, c, d, e, f) {
    let g; const h = sjcl.bitArray; g = h.f; const k = b.length; const l = h.bitLength(b); let n = k / 50; const m = n; c = h.concat([h.partial(8, f - 1)], c).concat([0, 0, 0]).slice(0, 4); d = h.bitSlice(g(d, a.encrypt(c)), 0, e); if (!k) return { tag: d, data: [] }; for (g = 0; g < k; g += 4) {
...
sjcl.beware["CBC mode is dangerous because it doesn't protect message integrity."] = function () {
  sjcl.mode.cbc = {
    ...
    encrypt: function (a, b, c, d) {
      if (d && d.length) throw new sjcl.exception.invalid("cbc can't authenticate data"); if (sjcl.bitArray.bitLength(c) !== 128) throw new sjcl.exception.invalid('cbc iv must be 128 bits'); const e = sjcl.bitArray; const f = e.f; let g = e.bitLength(b); let h = 0; const k = []; if (g & 7) throw new sjcl.exception.invalid('pkcs#5 padding only works for multiples of a byte'); for (d = 0; h + 128 <= g; d += 4, h += 128) {
        c = a.encrypt(f(c,
          b.slice(d, d + 4))), k.splice(d, 0, c[0], c[1], c[2], c[3])
      }g = 0x1010101 * (16 - (g >> 3 & 15)); c = a.encrypt(f(c, e.concat(b, [g, g, g, g]).slice(d, d + 4))); k.splice(d, 0, c[0], c[1], c[2], c[3]); return k
    },
    decrypt: function (a, b, c, d) {
      if (d && d.length) throw new sjcl.exception.invalid("cbc can't authenticate data"); if (sjcl.bitArray.bitLength(c) !== 128) throw new sjcl.exception.invalid('cbc iv must be 128 bits'); if (sjcl.bitArray.bitLength(b) & 127 || !b.length) throw new sjcl.exception.corrupt('cbc ciphertext must be a positive multiple of the block size')
      const e = sjcl.bitArray; const f = e.f; let g; const h = []; for (d = 0; d < b.length; d += 4)g = b.slice(d, d + 4), c = f(c, a.decrypt(g)), h.splice(d, 0, c[0], c[1], c[2], c[3]), c = g; g = h[d - 1] & 255; if (g === 0 || g > 16) throw new sjcl.exception.corrupt('pkcs#5 padding corrupt'); c = 0x1010101 * g; if (!e.equal(e.bitSlice([c, c, c, c], 0, 8 * g), e.bitSlice(h, 32 * h.length - 8 * g, 32 * h.length))) throw new sjcl.exception.corrupt('pkcs#5 padding corrupt'); return e.bitSlice(h, 0, 32 * h.length - 8 * g)
    }
  }
}
sjcl.mode.ocb2 = {
  ...
  encrypt: function (a, b, c, d, e, f) {
    if (sjcl.bitArray.bitLength(c) !== 128) throw new sjcl.exception.invalid('ocb iv must be 128 bits'); let g; const h = sjcl.mode.ocb2.S; const k = sjcl.bitArray; const l = k.f; let n = [0, 0, 0, 0]; c = h(a.encrypt(c)); let m; let p = []; d = d || []; e = e || 64; for (g = 0; g + 4 < b.length; g += 4)m = b.slice(g, g + 4), n = l(n, m), p = p.concat(l(c, a.encrypt(l(c, m)))), c = h(c); m = b.slice(g); b = k.bitLength(m); g = a.encrypt(l(c, [0, 0, 0, b])); m = k.clamp(l(m.concat([0, 0, 0]), g), b); n = l(n, l(m.concat([0, 0, 0]), g)); n = a.encrypt(l(n, l(c, h(c))))
    d.length && (n = l(n, f ? d : sjcl.mode.ocb2.pmac(a, d))); return p.concat(k.concat(m, k.clamp(n, e)))
  },
  decrypt: function (a, b, c, d, e, f) {
    if (sjcl.bitArray.bitLength(c) !== 128) throw new sjcl.exception.invalid('ocb iv must be 128 bits'); e = e || 64; const g = sjcl.mode.ocb2.S; const h = sjcl.bitArray; const k = h.f; let l = [0, 0, 0, 0]; let n = g(a.encrypt(c)); let m; let p; const r = sjcl.bitArray.bitLength(b) - e; let q = []; d = d || []; for (c = 0; c + 4 < r / 32; c += 4)m = k(n, a.decrypt(k(n, b.slice(c, c + 4)))), l = k(l, m), q = q.concat(m), n = g(n); p = r - 32 * c; m = a.encrypt(k(n, [0, 0, 0, p])); m = k(m, h.clamp(b.slice(c), p).concat([0,
  ...
  pmac: function (a, b) {
    let c; const d = sjcl.mode.ocb2.S; const e = sjcl.bitArray; const f = e.f; let g = [0, 0, 0, 0]; var h = a.encrypt([0, 0, 0, 0]); var h = f(h, d(d(h))); for (c = 0; c + 4 < b.length; c += 4)h = d(h), g = f(g, a.encrypt(f(h, b.slice(c, c + 4)))); c = b.slice(c); e.bitLength(c) < 128 && (h = f(h, d(h)), c = e.concat(c, [-2147483648, 0, 0, 0])); g = f(g, c)
...
sjcl.mode.gcm = {
  name: 'gcm',
  encrypt: function (a, b, c, d, e) { const f = b.slice(0); b = sjcl.bitArray; d = d || []; a = sjcl.mode.gcm.C(!0, a, f, d, c, e || 128); return b.concat(a.data, a.tag) },
  decrypt: function (a, b, c, d, e) { let f = b.slice(0); const g = sjcl.bitArray; const h = g.bitLength(f); e = e || 128; d = d || []; e <= h ? (b = g.bitSlice(f, h - e), f = g.bitSlice(f, 0, h - e)) : (b = f, f = []); a = sjcl.mode.gcm.C(!1, a, f, d, c, e); if (!g.equal(a.tag, b)) throw new sjcl.exception.corrupt("gcm: tag doesn't match"); return a.data },
  ka: function (a, b) {
    let c; let d; let e; let f; let g; const h = sjcl.bitArray.f; e = [0, 0,
  ...
  j: function (a, b, c) { let d; const e = c.length; b = b.slice(0); for (d = 0; d < e; d += 4)b[0] ^= 0xffffffff & c[d], b[1] ^= 0xffffffff & c[d + 1], b[2] ^= 0xffffffff & c[d + 2], b[3] ^= 0xffffffff & c[d + 3], b = sjcl.mode.gcm.ka(b, a); return b },
  C: function (a, b, c, d, e, f) {
...
sjcl.prng = function (a) { this.c = [new sjcl.hash.sha256()]; this.m = [0]; this.P = 0; this.H = {}; this.N = 0; this.U = {}; this.Z = this.g = this.o = this.ha = 0; this.b = [0, 0, 0, 0, 0, 0, 0, 0]; this.i = [0, 0, 0, 0]; this.L = void 0; this.M = a; this.D = !1; this.K = { progress: {}, seeded: {} }; this.u = this.ga = 0; this.I = 1; this.J = 2; this.ca = 0x10000; this.T = [0, 48, 64, 96, 128, 192, 0x100, 384, 512, 768, 1024]; this.da = 3E4; this.ba = 80 }
sjcl.prng.prototype = {
  randomWords: function (a, b) {
    const c = []; let d; d = this.isReady(b); let e; if (d === this.u) throw new sjcl.exception.notReady("generator isn't seeded"); if (d & this.J) {
      d = !(d & this.I); e = []; let f = 0; let g; this.Z = e[0] = (new Date()).valueOf() + this.da; for (g = 0; g < 16; g++)e.push(0x100000000 * Math.random() | 0); for (g = 0; g < this.c.length && (e = e.concat(this.c[g].finalize()), f += this.m[g], this.m[g] = 0, d || !(this.P & 1 << g)); g++);this.P >= 1 << this.c.length && (this.c.push(new sjcl.hash.sha256()), this.m.push(0)); this.g -= f; f > this.o && (this.o =
f); this.P++; this.b = sjcl.hash.sha256.hash(this.b.concat(e)); this.L = new sjcl.cipher.aes(this.b); for (d = 0; d < 4 && (this.i[d] = this.i[d] + 1 | 0, !this.i[d]); d++);
    } for (d = 0; d < a; d += 4)(d + 1) % this.ca === 0 && y(this), e = z(this), c.push(e[0], e[1], e[2], e[3]); y(this); return c.slice(0, a)
  },
  setDefaultParanoia: function (a, b) { if (a === 0 && b !== 'Setting paranoia=0 will ruin your security; use it only for testing') throw new sjcl.exception.invalid('Setting paranoia=0 will ruin your security; use it only for testing'); this.M = a },
...
(k = 1)
      ...
      } if (!k) { if (void 0 === b) for (c = b = 0; c < a.length; c++) for (e = a[c]; e > 0;)b++, e = e >>> 1; this.c[g].update([d, this.N++, 2, b, f, a.length].concat(a)) } break; case 'string':void 0 === b && (b = a.length); this.c[g].update([d, this.N++, 3, b, f, a.length]); this.c[g].update(a); break; default:k = 1
    } if (k) throw new sjcl.exception.bug('random: addEntropy only supports number, array of numbers or string'); this.m[g] += b; this.g += b; h === this.u && (this.isReady() !== this.u && A('seeded', Math.max(this.o, this.g)), A('progress', this.getProgress()))
  },
  isReady: function (a) { a = this.T[void 0 !== a ? a : this.M]; return this.o && this.o >= a ? this.m[0] > this.ba && (new Date()).valueOf() > this.Z ? this.J | this.I : this.I : this.g >= a ? this.J | this.u : this.u },
  getProgress: function (a) { a = this.T[a || this.M]; return this.o >= a ? 1 : this.g > a ? 1 : this.g / a },
  startCollectors: function () {
    if (!this.D) {
      this.a = { loadTimeCollector: B(this, this.ma), mouseCollector: B(this, this.oa), keyboardCollector: B(this, this.la), accelerometerCollector: B(this, this.ea), touchCollector: B(this, this.qa) }; if (window.addEventListener) {
  ...
  stopCollectors: function () {
    this.D && (window.removeEventListener
      ? (window.removeEventListener('load', this.a.loadTimeCollector, !1), window.removeEventListener('mousemove', this.a.mouseCollector, !1), window.removeEventListener('keypress', this.a.keyboardCollector, !1), window.removeEventListener('devicemotion', this.a.accelerometerCollector, !1), window.removeEventListener('touchmove', this.a.touchCollector, !1))
  ...
  removeEventListener: function (a, b) { let c; let d; const e = this.K[a]; const f = []; for (d in e)e.hasOwnProperty(d) && e[d] === b && f.push(d); for (c = 0; c < f.length; c++)d = f[c], delete e[d] },
  la: function () { C(this, 1) },
  oa: function (a) { let b, c; try { b = a.x || a.clientX || a.offsetX || 0, c = a.y || a.clientY || a.offsetY || 0 } catch (d) { c = b = 0 }b != 0 && c != 0 && this.addEntropy([b, c], 2, 'mouse'); C(this, 0) },
  qa: function (a) {
    a =
a.touches[0] || a.changedTouches[0]; this.addEntropy([a.pageX || a.clientX, a.pageY || a.clientY], 1, 'touch'); C(this, 0)
  },
  ma: function () { C(this, 2) },
  ea: function (a) { a = a.accelerationIncludingGravity.x || a.accelerationIncludingGravity.y || a.accelerationIncludingGravity.z; if (window.orientation) { const b = window.orientation; typeof b === 'number' && this.addEntropy(b, 1, 'accelerometer') }a && this.addEntropy(a, 2, 'accelerometer'); C(this, 0) }
}
function A (a, b) { let c; const d = sjcl.random.K[a]; const e = []; for (c in d)d.hasOwnProperty(c) && e.push(d[c]); for (c = 0; c < e.length; c++)e[c](b) } function C (a, b) { typeof window !== 'undefined' && window.performance && typeof window.performance.now === 'function' ? a.addEntropy(window.performance.now(), b, 'loadtime') : a.addEntropy((new Date()).valueOf(), b, 'loadtime') } function y (a) { a.b = z(a).concat(z(a)); a.L = new sjcl.cipher.aes(a.b) } function z (a) { for (let b = 0; b < 4 && (a.i[b] = a.i[b] + 1 | 0, !a.i[b]); b++);return a.L.encrypt(a.i) }
function B (a, b) { return function () { b.apply(a, arguments) } }sjcl.random = new sjcl.prng(6)
a:try {
  let D, E, F, G; if (G = typeof module !== 'undefined' && module.exports) { let H; try { H = require('crypto') } catch (a) { H = null }G = E = H } if (G && E.randomBytes)D = E.randomBytes(128), D = new Uint32Array((new Uint8Array(D)).buffer), sjcl.random.addEntropy(D, 1024, "crypto['randomBytes']"); else if (typeof window !== 'undefined' && typeof Uint32Array !== 'undefined') {
...
sjcl.json = {
  ...
  ja: function (a, b, c, d) {
    c = c || {}; d = d || {}; const e = sjcl.json; const f = e.h({ iv: sjcl.random.randomWords(4, 0) }, e.defaults); let g; e.h(f, c); c = f.adata; typeof f.salt === 'string' && (f.salt = sjcl.codec.base64.toBits(f.salt)); typeof f.iv === 'string' && (f.iv = sjcl.codec.base64.toBits(f.iv)); if (!sjcl.mode[f.mode] || !sjcl.cipher[f.cipher] || typeof a === 'string' && f.iter <= 100 || f.ts !== 64 && f.ts !== 96 && f.ts !== 128 || f.ks !== 128 && f.ks !== 192 && f.ks !== 0x100 || f.iv.length < 2 ||
f.iv.length > 4) throw new sjcl.exception.invalid('json encrypt: invalid parameters'); typeof a === 'string' ? (g = sjcl.misc.cachedPbkdf2(a, f), a = g.key.slice(0, f.ks / 32), f.salt = g.salt) : sjcl.ecc && a instanceof sjcl.ecc.elGamal.publicKey && (g = a.kem(), f.kemtag = g.tag, a = g.key.slice(0, f.ks / 32)); typeof b === 'string' && (b = sjcl.codec.utf8String.toBits(b)); typeof c === 'string' && (f.adata = c = sjcl.codec.utf8String.toBits(c)); g = new sjcl.cipher[f.cipher](a); e.h(d, f); d.key = a; f.ct = f.mode === 'ccm' && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm &&
b instanceof ArrayBuffer
      ? sjcl.arrayBuffer.ccm.encrypt(g, b, f.iv, c, f.ts)
      : sjcl.mode[f.mode].encrypt(g, b, f.iv, c, f.ts); return f
  },
  encrypt: function (a, b, c, d) { const e = sjcl.json; const f = e.ja.apply(e, arguments); return e.encode(f) },
  ia: function (a, b, c, d) {
    c = c || {}; d = d || {}; const e = sjcl.json; b = e.h(e.h(e.h({}, e.defaults), b), c, !0); let f, g; f = b.adata; typeof b.salt === 'string' && (b.salt = sjcl.codec.base64.toBits(b.salt)); typeof b.iv === 'string' && (b.iv = sjcl.codec.base64.toBits(b.iv)); if (!sjcl.mode[b.mode] || !sjcl.cipher[b.cipher] || typeof a ===
...
'ccm' && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && b.ct instanceof ArrayBuffer
      ? sjcl.arrayBuffer.ccm.decrypt(g, b.ct, b.iv, b.tag, f, b.ts)
      : sjcl.mode[b.mode].decrypt(g, b.ct, b.iv, f, b.ts); e.h(d, b); d.key = a; return c.raw === 1 ? f : sjcl.codec.utf8String.fromBits(f)
  },
  decrypt: function (a, b, c, d) { const e = sjcl.json; return e.ia(a, e.decode(b), c, d) },
  encode: function (a) {
...
b + '":'; d = ','; switch (typeof a[b]) { case 'number':case 'boolean':c += a[b]; break; case 'string':c += '"' + escape(a[b]) + '"'; break; case 'object':c += '"' + sjcl.codec.base64.fromBits(a[b], 0) + '"'; break; default:throw new sjcl.exception.bug('json encode: unsupported type') }
  ...
  h: function (a, b, c) { void 0 === a && (a = {}); if (void 0 === b) return a; for (const d in b) if (b.hasOwnProperty(d)) { if (c && void 0 !== a[d] && a[d] !== b[d]) throw new sjcl.exception.invalid('required parameter overridden'); a[d] = b[d] } return a },
  sa: function (a, b) { const c = {}; let d; for (d in a)a.hasOwnProperty(d) && a[d] !== b[d] && (c[d] = a[d]); return c },
  ra: function (a,
